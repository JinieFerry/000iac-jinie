...
## 03. 시작 템플릿 : 0306-launch-template-01
...
## 04. 대상그룹 생성
<img width="1002" height="725" alt="image" src="https://github.com/user-attachments/assets/2f2105b5-d5e5-4f5d-a764-f0c2fce4dda2" />

## 05. ALB 생성

<img width="1105" height="327" alt="image" src="https://github.com/user-attachments/assets/225537d2-436c-4ea0-9a74-638007736bc3" />

<img width="1381" height="768" alt="image" src="https://github.com/user-attachments/assets/16dc8b7e-dcbd-43c9-961f-1f17c1a97ad4" />

# 4444      

<img width="1420" height="790" alt="image" src="https://github.com/user-attachments/assets/6c1c3a25-9eca-425b-a9eb-220571e9ca7c" />

+ 보안그룹 생성
<img width="1373" height="543" alt="image" src="https://github.com/user-attachments/assets/85dc421f-4590-4e9b-99b4-a233c4ee9bba" />

<img width="1370" height="694" alt="image" src="https://github.com/user-attachments/assets/15d1e0ea-8c1e-4b24-839e-0eded1bb2bd2" />
...
+ 라우팅 테이블
 0306-net-01-rtb-public   
 0306-net-01-rtb-private-2a   
 0306-net-01-rtb-private-2b   
...
1) 0306-net-01-subnet-public1-ap-northeast-2a : 퍼블릭 IPv4 주소 할당 활성화

+ 자동 할당 IP 설정 : 퍼블릭 IPv4 주소 자동 할당 활성화 : 체크표시 켜기 -> 저장

2) 0306-net-01-subnet-public2-ap-northeast-2b : 퍼블릭 IPv4 주소 할당 활성화
...
+ 기본
  + 이름 : 0306-sg-alb-01    
  + 설명 : 0306-sg-alb-01
  + VPC : 0306-net-01-vpc <- 보안그룹은 같은 vpc 안에서만 서로 참조 가능하므로 통일
...
+ 인바운드 규칙 : ALB -> EC2만 허용
  + 유형 :  HTTP
  + 포트 범위 : 80포트
  + 소스 : 사용자 지정 -> 0306-sg-alb-01 (같은 vpc 안에 있어야 선택 가능)
...
+ 네트워크 매핑
   + VPC : 0306-net-01-vpc 선택 10.30.0.0/16
...
+ 네트워크 설정
   + VPC : 편집 눌러서 0306-net-01-vpc (10.30.0.0/16)으로 설정
   + 서브넷 (첫번째 EC2) : 0306-net-01-subnet-private1 (10.30.10.0/24)
   + 퍼블릭 IP 자동 할당 : 비활성화
   + 방화벽 : 기존 보안 그룹 선택
   + 일반 보안 그룹 : 0306-sg-ec2-01 
...
**EC2 > 대상그룹 > 0306-tg-web-01 선택 > 대상 등록 클릭**

+ 대상 등록 > 사용 가능한 인스턴스
   + 0306-ec2-web-01과 0305-ec2-web-02 선택 
+ '아래에 보류 중인 것으로 포함' 클릭
...
+ DNS 이름 복사
```
0306-alb-web-01-531676862.ap-northeast-2.elb.amazonaws.com
```
...
+ 1단계 : 시작 템플릿 선택
   + 이름 : 0306-asg-web-01
   + 시작 템플릿 : 0306-launch-templete-01
...
## 12. ALB 접속 테스트
**EC2 > 로드밸런서 > 0306-alb-web-01 선택**
+ DNS 이름 복사 : 0306-alb-web-01-531676862.ap-northeast-2.elb.amazonaws.com
+ 브라우저 접속
