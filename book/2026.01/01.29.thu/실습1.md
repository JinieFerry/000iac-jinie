...
```
지니결과
<img width="946" height="463" alt="image" src="https://github.com/user-attachments/assets/72ab3f33-e251-4648-8e22-e66f9d139091" />
```
master@vm1:~$ ls
또는  before0121  sej  snap  sudo  vi  wsd
master@vm1:~$ cd sej/
master@vm1:~/sej$ ls
000iac-jinie  silsoup
master@vm1:~/sej$ cd silsoup/
master@vm1:~/sej/silsoup$ ls
0121ss  0129ss
master@vm1:~/sej/silsoup$ cd 0129ss/
master@vm1:~/sej/silsoup/0129ss$ ls
master@vm1:~/sej/silsoup/0129ss$ sudo mkdir -p /etc/apt/keyrings
[sudo] password for master: 
master@vm1:~/sej/silsoup/0129ss$ curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo gpg --dearmor -o /etc/apt/keyrings/docker.gpg
master@vm1:~/sej/silsoup/0129ss$ echo \
  "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.gpg] https://download.docker.com/linux/ubuntu \
  $(lsb_release -cs) stable" | sudo tee /etc/apt/sources.list.d/docker.list > /dev/null
master@vm1:~/sej/silsoup/0129ss$ 
...
04.PrivateCloudInfra/01.도커및쿠버네티스설치
/012.k8s설치하기.md
...
#시도1: 네트워크 서버가 연결을 끊었으니 한국서버가 아닌 메인서버로 연결
sudo sed -i 's|kr.archive.ubuntu.com|archive.ubuntu.com|g' /etc/apt/sources.list
#결과1: 같은 에러
  Error reading from server. Remote end closed connection [IP: 210.117.237.2 80]
E: Failed to fetch http://kr.archive.ubuntu.com/ubuntu/pool/main/l/linux-firmware/linux-firmware_20240318.git3b128b60-0ubuntu2.23_amd64.deb  Error reading from server. Remote end closed connection [IP: 210.117.237.2 80]
E: Unable to fetch some archives, maybe run apt-get update or try with --fix-missing?
# 일단 두고 넘어가기 _ 나중에 업그레이드
```
```
# 2. Swap 메모리 비활성화 (k8s 스케줄러가 리소스를 정확히 계산하기 위해 필수)
sudo swapoff -a
sudo sed -i '/swap/s/^/#/' /etc/fstab # 재부팅 시에도 적용되도록 설정
#fstab: 파일시스템 테이블 s:exchange, #:맨처음을 #으로 바꿈=주석처리=막힘

# 3. 네트워크 브리지 모듈 로드 및 커널 파라미터 설정
cat <<EOF | sudo tee /etc/modules-load.d/k8s.conf #tee: EOF칠 때 까지 그 파일에 입력해
overlay
br_netfilter
EOF

# 중요한 파일이니까 확인
master@vm1:~/sej/silsoup/0129ss$ cat /etc/modules-load.d/k8s.conf
overlay
br_netfilter
...
#확인
master@vm1:~/sej/silsoup/0129ss$ cat /etc/sysctl.d/k8s.conf
net.bridge.bridge-nf-call-iptables  = 1
net.bridge.bridge-nf-call-ip6tables = 1
net.ipv4.ip_forward      
...
containerd config default | sudo tee /etc/containerd/config.toml > /dev/null #버리라는 뜻(보여줄 필요없는 것)
...
### 4. 마스터 노드(Control Plane) 초기화 #vm1,vm2에서 생략 -> 하면 마스터 2개 생김
...
```
init하면 10네트워크로 바껴버림
=> vmmaster01의 'docker 복원 후 업그레이드 완료를 복제해서' vm1을 생성하기 
`which docker`로 도커 설치확인
<= vm3로 복제하면 안되는 이유: vm3가 mother가 되니까:마더는 전체를 다 가지고 있는 애로 마더를 삼아야 함
vm1은 이전에 사용했던 이름으로 인식 c/사용자/.virtualbox 
