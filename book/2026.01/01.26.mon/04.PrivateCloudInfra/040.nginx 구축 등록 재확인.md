...
# 4. 이미지 빌드 (-t는 태그/이름 설정) 컴퓨터가 알아듣는 바이너리 언어로 바꿔주기(빌드)
docker build -t my-web-app:v1 .

```

### 2. 이미지 확인 및 상태 해설 : 워드로 친 작업 이미지 만들기(컴퓨터가 알아 듣게)
...
### 3. 포트 포워딩을 이용한 컨테이너 실행(위 까지는 이제 저장 된 것, 실행시켜야 함)
...
docker tag my-web-app:v1 [본인_도커허브_ID]/my-web-app:v1 #이미지 이름 지었어도 이미지는 여러 사람이 가져가는 거니까 이름을 또 지어주는 것
