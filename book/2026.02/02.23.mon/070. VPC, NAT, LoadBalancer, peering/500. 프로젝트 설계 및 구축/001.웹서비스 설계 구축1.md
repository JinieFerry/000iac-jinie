...
: 대부분은 EKS에 있음 
프론트엔드는 라이브러리 골라서 도커 허브 가서 오리지널 엔진 하나 떼서 빌드,런 테스트 후 올려서 회사 레퍼지토리에서 직접 받아가도록 올림
...
│   ├── envs/                       # envrionmets
│   │   └── prod-apne2/             # 한국 운영 환경 asia pasipic network
│   │       ├── main.tf
│   │       ├── variables.tf        # 변수는 박아두지 않고 밖으로 빼 둠
│   │       └── outputs.tf          # K8s로 넘겨줄 DB 엔드포인트 등 출력 : 테라폼은 항상 아웃풋을 만듦 (잘 끝냈는지 혹은 ip가 뭔지 알려줌)
│   └── modules/
│       ├── network/                # VPC, Subnet, NAT 등
│       ├── eks/                    # EKS Cluster, Node Group (v1.30) : elastic kubernetes system
│       └── rds/                    # RDS MySQL (v8.0.36) : DB mySQL,MariaDb도 상관없음 (쓰고 싶은 것 쓰기): 국가사업에서 MariaDB로 많이 하는 추세임
...
```
+  = Maria DB와 MySQL둘 다 했다고 하거나 마리아 DB로 했다고 하기 (오라클이 가장 좋지만 가격이 10배 정도)
+  우리나라 보안 회사는 postSQL 사용함
---
