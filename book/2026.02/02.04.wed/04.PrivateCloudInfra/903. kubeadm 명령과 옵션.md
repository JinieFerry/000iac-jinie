...
| `kubeadm token create --print-join-command` | **token** | `create`, `list`, `delete`, `--print-join-command` | (메인 마스터에서 토큰 발행함)노드 합류에 필요한 **인증 토큰** 관리 및 조인 명령어 재생성 |
...
컨트롤러가 배정해두면 스케줄러가 배정함?-> API가 해당 큐블렛에 전달함 -> CNI 가상 네트워크-무작위 혹은 순서대로 배정(DHCP처럼-순서가 있음) : Flannel, Calico
---
