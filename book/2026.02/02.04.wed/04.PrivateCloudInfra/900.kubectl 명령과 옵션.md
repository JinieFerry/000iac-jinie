<img width="690" height="490" alt="image" src="https://github.com/user-attachments/assets/90ad4db7-5798-474a-b538-45d0c1f876e0" />    


바로 pod로 물리는 것 같지만(표현상), 실제로는 물리적으로 닉에 먼저 물려줘야 pod로 감
>외부 트래픽은 논리적으로는 Pod로 전달되는 것처럼 보이지만,
>실제 물리 경로는 노드의 NIC를 통해 유입된 후
>커널 네트워크 스택과 CNI 구성을 거쳐
>최종적으로 Pod의 네트워크 네임스페이스로 전달된다.
---
...
| --- | --- | --- | --- |
| `kubectl apply -f app.yaml` | **apply** | `-f` (파일 지정) |(apply:실행,-f:특정파일) 설정 파일을 읽어 리소스 생성 및 상태 업데이트 |
...
| `kubectl exec -it my-pod -- bash` | **exec** | `-it` (터미널 접속) |(pod 내부 exec로 접근가능) 실행 중인 컨테이너 내부에 접속하여 명령어 실행 |
| `kubectl delete pod my-pod --force` | **delete** |(pod,deployment,service 작업하면서 지워버릇하기) `-f`, `--force`, `--grace-period=0` | 리소스 삭제 (강제 삭제 및 즉시 제거 옵션) |
...
| `kubectl cp ./file my-pod:/tmp` | **cp** | (Source) (Dest) |(파일만 보내고 싶을 때)인덱스파일고칠 때 많이 씀! 로컬 파일 시스템과 컨테이너 간 파일 복사 |
...
# 서비스는 실제 일은 pod가 다 함 : pod가 서비스 하는 포트가 target-port, --port는 밖에서 요청 받을 때 받는 포트 : 문이 두개 있어서 80을 80으로 바꿔줘야 하는 것(물리적으로 노드에 먼저 들어가고:외부에서는 내부 상황을 모름 어떤 프로그램 만날지 모름: 첫 접속 관문 80:아무것도 안하는 것 , -> 파드 접속:nginx는 보통 80, 백엔드는 보통 8080에서 )
```
