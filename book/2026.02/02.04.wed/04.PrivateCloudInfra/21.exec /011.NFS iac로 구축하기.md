...
- name: NFS 스토리지 서버 자동 구성:NFS따로 apt로 깔 수도 있지만 yaml 루트권한으로 설치
  # 우린 아직 인벤토리에 없음
  ...
  tasks:
    ...
    - name: 2. 공유 디렉토리 생성 (/srv/nfs/shared_data)
      file:
        path: /srv/nfs/shared_data    # 서버 측 
        state: directory
        mode: '0777' # 실습 편의를 위한 전권한 부여 #실습용:읽고 쓰고 실행하기 <- 추후 목적에 따라 변경

    - name: 3. NFS 내보내기 설정 (Exports)
      lineinfile:
        path: /etc/exports      # 내보내는 pod 측
        line: "/srv/nfs/shared_data *(rw,sync,no_subtree_check,no_root_squash)"
        state: present

    - name: 4. 설정 적용 및 서비스 재시작
      shell: exportfs -ra
      notify: Restart NFS Service # 위의 명령어 선험적으로 다 실행 한 후에 한번만 실행 됨

  #ansible handlers는 task실행 중 즉시 실행되지 않고, play 종료 시점에서 변경 사항이 있었을 경우, 단 한 번 실행 됨
  handlers:
...
resource "kubernetes_persistent_volume" "nfs_pv" {
  ...
  spec {
    ...
    persistent_volume_source {
      nfs {
        path   = "/srv/nfs/shared_data"   #가장 중요
...
# PersistentVolumeClaim (PVC): 애플리케이션의 스토리지 자원 요청
resource "kubernetes_persistent_volume_claim" "nfs_pvc" {
  metadata {
    name = "nfs-pvc-request"
  }
  spec {
    access_modes = ["ReadWriteMany"]   # 가장 중요
    resources {
      requests = {
        storage = "5Gi"  # PVC가 요청하는 스토리지 용량
      }
    }
    storage_class_name = "nfs-manual"
  }
