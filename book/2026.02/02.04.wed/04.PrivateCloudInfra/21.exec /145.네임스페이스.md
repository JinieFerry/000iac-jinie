...
<img width="693" height="449" alt="image" src="https://github.com/user-attachments/assets/42d56860-ba53-401d-aeed-d2335d3a3dac" />
+ 트래픽이 많아서 백엔드는 빠르게 전달만 하려고 앞으로 서비를 빼둔 것! (그림의 예시)
+ 클러스터 분리해서 하나 더 만들면 힘드니까 네임스페이스를 나눔: 접근하지 못 하게 할 때 (개발팀에서 이슈 생기면 개발팀 접근 못 하게 분리함)
쿠버네티스는 컨트롤 플레인 하나 당 클러스터 하나를 갖기 때문에
>쿠버네티스에서 하나의 Control Plane는
>하나의 클러스터를 관리하도록 설계되어 있다.
#### Namespace를 쓰는 이유: 클러스터를 여러 개 만들면 Control Plane도 여러 개
>관리/비용/운영 부담 증가하기 때문에 하나의 클러스터를 유지하는 대신, 네임스페스를 나눠서 논리적으로 분리함 : 접근통제, 팀별 격리(보통 개발팀,특히 신입 접근을 막음)
---
