...
apiVersion: v1
kind: PersistentVolume  #kind에는 정해진 이름
metadata:
  name: nfs-pv-5gb      #연결할 때 호출하는 네임 (IaC 쿠버네티스에는 name에 주로 쓰는 _ 대신 - 많이 사용하는 추세)
spec:
  capacity:
  ...
  nfs:
    server: 192.168.x.50 # VM 50번의 IP 주소로 수정 : 자기 마음대로 
