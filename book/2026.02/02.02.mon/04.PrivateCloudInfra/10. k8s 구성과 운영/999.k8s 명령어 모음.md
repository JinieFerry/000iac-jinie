...
** 쉘 설정 파일(~/.bashrc)에 자동 완성 스크립트 추가 : 재부팅 후에 접속하고 프로파일 띄울 때 .profile이 먼저-> 그 다움 .bashrc가 실행 됨
...
결과
```
master@vmmaster1:~$ sudo apt-get install bash-completion -y
...
The following packages were automatically installed and are no longer required:
  docker-ce-rootless-extras libllvm19 libslirp0 pigz slirp4netns
Use 'sudo apt autoremove' to remove them.
0 upgraded, 0 newly installed, 0 to remove and 24 not upgraded.
master@vmmaster1:~$ echo 'source <(kubectl completion bash)' >> ~/.bashrc
master@vmmaster1:~$ source ~/.bashrc
master@vmmaster1:~$ 
```
...
#sudo는 단순히 루트 권한으로 들어가는 것 <>sudo -i 환경변수 다 불러올 수 있음
$ sudo -i
#kubelet: 각 노드의 대표: 파드를 넣고 뺌
# kubelet --version
:
...
#노드는 pc한 대, 파드가 노드 안에 들어았음
$ kubectl get all     # pod 정보와 서비스 정보
