...
가장 기본적인 배포 단위로, 하나 이상의 컨테이너를 포함합니다.
 (yaml은 IaC, 파드 아이피를 넣을 순 있지만 아이피 계속 바껴서 yaml파일 쓸 수가 없음. write하는 DB는 쓸 수 있지만 잘 안 넣음)   

```yaml
apiVersion: v1              # API 버전: 옛날 버전과 구분하기 위해 씀 환경파일에서 중요
kind: Pod                   # 객체 종류
metadata:
  name: nginx-pod           # 파드 이름: 이름을 보고 찾아가기 때문에 중요 : nginx여러개=도메인 여러개 가능
spec:
  containers:               # 컨테이너 설정
  - name: nginx             # 컨테이너 이름
...
kind: Service
metadata:
  name: internal-svc        # 서비스 이름 (내부 서비스)
spec:
  selector:                 # 대상 파드를 지정하는 라벨
    app: web                # 프록시가 대표 아이피로 보냄 : 대표=app (l2,l3가 헤더만 보듯이 프록시가 selector 봐서 앱:대표를 보냄) 
  ports:
  - protocol: TCP
    port: 80                # 서비스가 노출할 포트
...
### **9. Ingress (인그레스)**: 지금은 좀 가르칠 지 말지 애매한 시기:gate way api가 훨씬 나아서 많이 쓰느 중
