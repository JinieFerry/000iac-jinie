...
회사 로직에 따라 늘리는 것이 달라짐     
예: 스트리밍만 함 = 메모리 늘려, 이미지를 리터칭 해 = CPU GPU 늘려
실전에서는 노드 하나에 파드 한 두대 올림 : 로드 수집하기 위해서 사이드 카로는 부족함  : 성능평가 파드로 여유 줘서 하는 것 = CPU 많이 줘야 함
서비스는 하나지만 도와주는 파드 하나 있어야해서 노드 하나 당 파드 두개 정도    
IPS 프로텍트하는 / IDS 탐지하는 애들까지 보안까지 붙으면 세개 정도 올리게 됨    
IPS:탐지하는데 시간이 짧으면 코어 더 줌, IDS는 코어 더 줄 필요 없음    
