...
---
### 0. 기초 연습이어서 비교적 단순하지만, 실제로는 버전 관리를 위해 다 씀   
### 1. 디플로이먼트의 계층 구조
  ...
  template:                     # 5. [Pod 계층] 생성될 포드의 설계도 (Pod Template)
    ...
    spec:                       # 6. [Pod 계층] 포드 내부 컨테이너 상세 설정
      ...
      - name: nginx             #      * 컨테이너 이름 정의
        image: nginx:1.14.2     #      * 사용할 도커 이미지 및 버전 # 최근꺼 말고 수정해서 깔아서 쓰기
        ports:                  #      * 네트워크 포트 설정
...
## 1. 디플로이먼트 기본 생성 및 관리 : 실전에서는 명령형을 잘 안 쓰고 모두 yaml로 함
...
# -o yaml 만 쳐도 되는데 리다이렉션 주면 파일로 만들어 줌
# dry: 실제 실행은 아닌데 run해줘~
