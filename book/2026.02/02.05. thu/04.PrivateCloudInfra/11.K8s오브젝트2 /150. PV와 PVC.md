...
### 1. PV (Persistent Volume): 파드가 죽으면 데이터도 날라가니까    
...
### 2. PVC (Persistent Volume Claim) : 특정 파드들은 서비스 뿐 아니라 데이터 저장까지 해야 함   
...
4. **회수 (Reclaiming):** PVC를 삭제했을 때 남은 PV를 어떻게 처리할지 결정합니다. : 지금 쓰는 SAN을 그대로 백업하고 새 SAN을 사서 연결(옮기는 것 보다 효율적,5년 되면 속도가 더 빠른게 나옴)
...
metadata:                                           # 메타데이터 시작
  name: ebs-pv-storage                              # PV의 고유 이름 설정: 로직이 많아지면 속도,성능으로 이름 짓기도 함
