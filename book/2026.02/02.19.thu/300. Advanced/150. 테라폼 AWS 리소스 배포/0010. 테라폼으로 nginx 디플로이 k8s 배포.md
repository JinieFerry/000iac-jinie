      ...
      spec {
        container {
          ...
          resources {
            ...
            requests = {
              cpu    = "250m"
              memory = "50Mi"  # 밀리코어: 초 당, cpu는 1000으로 가기 때문에 250에서 500밀리 코어라고 봐야 함
            }
          }
        }
      }
...
resource "kubernetes_service" "nginx_svc" {
  ...
  spec {
    ...
    type = "NodePort" # 클라우드 환경인 경우 LoadBalancer(로드밸런서)아님 수정필요!
  }
}
...
배포 완료 후 확인이 필요한 정보를 화면에 출력합니다. : 아웃풋에 결과값을 저장함
```hcl
output "service_ip" {
