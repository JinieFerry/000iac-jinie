...
### 1. HostPath vs Local Path Provisioner 비교 : 프리비저닝: 말 그대로 pv -> pvc의 순서였던 것 호스트 패스와 달리 pvc만들면 자동으로 pv 만들어짐

| 구분 | HostPath | Local Path Provisioner |
| --- | --- | --- |
| **PV 생성 방식** | 수동 (Static) | 자동 (Dynamic):삭제만 안 할 뿐 퍼시스턴트 볼륨임` |
...
### 2. HostPath의 한계점
호스트패스는 박아넣는 유연성이 떨어져서 잘 안 쓰고 , 루트 연결 잘 안한다.     
HostPath는 노드의 특정 경로를 컨테이너에 직접 연결합니다. 하지만 다음과 같은 치명적인 단점이 있습니다.
호스트패스는 로컬이라 알 수 없음 = 연결할 수 없음   
* **노드 고정 불가**: 포드가 재시작되어 다른 워커 노드로 이동하면, 이전 노드에 있던 데이터에 접근할 수 없습니다.
* **보안 위험**: 포드가 호스트의 파일 시스템에 직접 접근하므로 보안상 취약할 수 있습니다. : 루트 디렉토리로 들어가면 바로 뚫릴 수 있음 : 시스템 접근 못 하도록 홈 밑에 둠 : 루트의 권한으로 한 다거나 루트의 영역으로 하는 일은 시스템 엔지니어는 거의 안 함 
* **수동 관리**: 모든 노드에 동일한 경로를 미리 만들어두거나, 특정 노드에 포드가 뜨도록 `nodeSelector`를 매번 설정해야 합니다. : 그래야 다른 노드에 갈 수 있음
...
kubectl get pods -n local-path-storage # -n: namespace

# 생성된 스토리지 클래스(StorageClass) 확인 :yaml 안에서 쓸거임 스토리지 클래스 이름 기억!
kubectl get sc
...
```yaml
# local-pvc.yaml #pv가 아니니까 pvc라고 이름 지음
apiVersion: v1
kind: PersistentVolumeClaim # 영구적 저장소로 씀 이것도
metadata:
  name: wb-local-pvc
  namespace: wb_prod
spec:
  accessModes:
    - ReadWriteOnce # 공유할 때 Many, 혼자 할 때 Once
  storageClassName: local-path  # 이 스토리지 클래스보고 내꺼인지 다른 사람꺼인지 보고 연결함
  resources:
    requests:
      storage: 1Gi #용량
