## Kubernetes 리소스 연결 및 환경 업데이트 : 외부로 하면 덮어씌워서 IaC의미가 없어지니까 py-pvc 만듦
  ...
  template:
    ...
    spec:
      ...
      - name: python-server
        ...
        workingDir: /data # workingDir=홈 디렉토리 말 하는 것
      ...
      - name: backend-storage
        persistentVolumeClaim:
          claimName: back-pvc   #back-pvd.yaml에서의 name과 맞아야 함
---
apiVersion: v1
kind: Service
metadata:
...
#파일 있는 디렉토리가서
cd
cd k8s/loosepjk get pvc 
kubectl apply -f py-pvc.yaml

cd /opt
ls
cd local-path-provisioner/
k get pvc back-pvc -n wb-prod -o jsonpath='{.spec.volumeName}'
```


2. **마운트 상태 확인**
```bash
kubectl describe pod py-web -n wb-prod
# Containers -> python-server -> Mounts 섹션 확인
```

+  확인하기 
  크롬에서 192.168.115.251:31080 <= 아무것도 없는 페이지로 덮어씌워져서 아무것도 안 뜸
<img width="914" height="440" alt="image" src="https://github.com/user-attachments/assets/20123505-38fc-439f-9fb9-33607c168ac2" />
+ 인덱스 안씌우고 가보기(예측:디렉토리 다 없어졌을 것)
